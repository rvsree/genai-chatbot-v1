# app/service/feature/react_single_agent/react_service.py
...
class ReactToolCallingAgent(ReactBaseAgent):
    async def synthesize_final_with_meta(
        ...
        temp = temperature if temperature is not None else getattr(_cfg, "openai_llm_temperature", 0.25)
        max_toks = max_tokens if max_tokens is not None else getattr(_cfg, "rag_max_tokens", 256)

        # context_notes already has "Allowed citations: ..." header as first line from base agent
        ctx_str = "\n".join(context_notes if isinstance(context_notes, list) else [str(context_notes)])

        messages = [
            {"role": "system", "content": react_prompts.REACT_SYSTEM_STRICT},
            {"role": "user", "content": react_prompts.REACT_USER_EXTRACT.format(question=variant_query, context=ctx_str)}
        ]
        ...
        # Replace placeholder and enforce at least one citation
